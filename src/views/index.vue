<template xmlns:v-bind="http://www.w3.org/1999/xhtml">
  <div class="wrapper" id="index">
    <data-hunter-header title="可视化之星大赛"/>
    <data-hunter-banner/>
    <data-hunter-bar active="1"/>
    <div class="content-wrapper">
      <section class="description">
        <p> 随着企业各类业务数据不断增长，如何通过数据挖掘商业价值，并以统一、高效的形式呈现给管理者，从而提高分析决策效率以及加快市场影响速度，已成为所有企业最为关注的话题。这其中，数据可视化是必不可少的技术之一。
        </p>
        <p>
          2018年10月，由DataHunter主办的首届数据可视化之星大赛火热开启！即日起至11月18日，通过DataHunter数据可视化分析平台制作并提交看板，就有机会成为DataHunter产品代言人，更有万元大奖等你拿！
        </p>
      </section>
      <div class="info-wrapper">
        <div class="info">
          <div class="title">比赛流程</div>
          <div class="process">
            <ul>
              <li>
                <div class="step">1</div>
                <p>报名提交</p>
                <p>(10月10日-11月18日)</p>
              </li>
              <li>
                <div class="step">2</div>
                <p>网民投票</p>
                <p>(10月10日-11月20日)</p>
              </li>
              <li>
                <div class="step">3</div>
                <p>专家评审</p>
                <p>(11月21日-11月23日)</p>
              </li>
              <li>
                <div class="step">4</div>
                <p>分数统计</p>
                <p>(11月24日)</p>
              </li>
              <li>
                <div class="step">5</div>
                <p>奖项公示</p>
                <p>(11月26日)</p>
              </li>
            </ul>
          </div>
          <div class="title">参赛要求</div>
          <div class="request">
            <ul>
              <li>
                <div>
                  <img src="@/assets/require/01.png">
                </div>
              </li>
              <li>
                <div>
                  <img src="@/assets/require/02.png">
                </div>
              </li>
              <li>
                <div>
                  <img src="@/assets/require/03.png">
                </div>
              </li>
            </ul>
          </div>
          <div class="title title-standard">评分标准</div>
          <div class="standard">
            <p> 评分实行百分制，即满分100分（占比：70%评委评分，30%网络投票）</p>
            <div class="standard__title">
              作品评分标准主要由三部分构成
            </div>
            <ul>
              <li>
                <div>
                  <img src="@/assets/standard/04.png">
                </div>
              </li>
              <li>
                <div>
                  <img src="@/assets/standard/05.png">
                </div>
              </li>
              <li>
                <div>
                  <img src="@/assets/standard/06.png">
                </div>
              </li>
            </ul>
          </div>
          <div class="title">数据要求</div>
          <div class="process__data">
            <p>1、参赛者可自行虚拟数据，或在确保数据脱敏的情况下使用自有数据，进行创意可视化展现。</p>
            <p>2、如参赛者没有虚拟数据，可使用DataHunter提供的示例数据：DataHunter所提供的数据，不得用于其他任何目的。</p>
            <p>3、参赛作品的知识产权归参赛作者所有。</p>
            <p>4、DataHunter有权免费地不受限制使用参赛作品（包括作品中所含的原始数据、设计、图表、照片、作品相关、作者信息）包括但不限于用于宣传品、指定及授权媒体发布、官方网站浏览及下载、 展览（含巡展）等活动项目。</p>
            <a href="./static/data.zip" download="data.zip">下载:demo数据</a>
          </div>
        </div>
      </div>
    </div>
    <div class="award">
      <div class="title">奖项设置</div>
      <ul>
        <li>
          <img src="@/assets/reward/1.png"/>
        </li>
        <li>
          <img src="@/assets/reward/2.png"/>
        </li>
        <li>
          <img src="@/assets/reward/3.png"/>
        </li>
      </ul>
      <ul>
        <li>
          <img src="@/assets/reward/4.png"/>
        </li>
        <li>
          <img src="@/assets/reward/5.png"/>
        </li>
      </ul>
    </div>
    <div class="judge">
      <div class="title">大赛评委</div>
      <ul class="judge__man clearfix">
        <li>
          <div class="judge__img">
            <img src="@/assets/guest/jiabin01.png">
          </div>
        </li>
        <li>
          <div class="judge__img">
            <img src="@/assets/guest/jiabin02.png">
          </div>
        </li>
        <li>
          <div class="judge__img">
            <img src="@/assets/guest/jiabin03.png">
          </div>
        </li>
        <li>
          <div class="judge__img">
            <img src="@/assets/guest/jiabin04.png">
          </div>
        </li>
        <li>
          <div class="judge__img">
            <img src="@/assets/guest/jiabin05.png">
          </div>
        </li>
      </ul>
      <div class="judge__rule">
        <p>组委会权利</p>
        <ul>
          <li>大赛组委会保留修改比赛作品提交的截止日期、获奖名单公布日期的权利</li>
          <li>大赛组委会保留收回或拒绝授予选手奖项的权利</li>
          <li>大赛组委会保留对该大赛规则修订的权利，并拥有对大赛规则的最终解释权。</li>
        </ul>
      </div>
    </div>
    <div class="organizer_wrapper">
      <div class="organizer">
        <div class="title">主办单位</div>
        <div class="organize organizer_center">
          <img src="@/assets/thanks/thank1.png" alt="">
        </div>
        <div class="title">联合主办</div>
        <div class="organize organizer_center">
          <img src="@/assets/thanks/thank2.png" alt="">
        </div>
        <div class="title">特别鸣谢</div>
        <!--<div class="organize organizer_center">-->
          <!--<img src="@/assets/thanks/thank3.png" alt="">-->
        <!--</div>-->
        <ul>
          <li>
            <div class="organize">
              <img src="@/assets/thanks/thank3.png" alt="">
            </div>
          </li>
          <li>
            <div class="organize">
              <img src="@/assets/thanks/thank4.png" alt="">
            </div>
          </li>
        </ul>
      </div>
    </div>
    <data-hunter-footer/>
  </div>
</template>

<script type="text/ecmascript-6">
  export default{
    mounted() {
      document.getElementById("index").style.minHeight = document.documentElement.clientHeight + 'px';
    },
    data() {
      return {
      };
    },
  };
</script>

<style scoped lang="scss" rel="stylesheet/scss">
  #index {
    min-width: 1280px;
    margin: 0 auto;
    padding-bottom: 90px;
    background: rgb(16, 33, 49);
  }


  .description {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    margin: 0 auto;
    width: 62.5%;
    min-width: 1200px;
    height: 332px;
    font-size: 16px;
    color: #fff;
    line-height: 28px;
    p {
      width: 100%;
      margin-bottom: 30px;
    }
  }

  .content-wrapper {
    background: url("../../static/pc_bg.png") 100% 80px no-repeat;
    background-size: 100% 100%;
  }

  .info-wrapper {
    background-color: rgba(14,44,73,.6);
  }

  .info {
    width: 62.5%;
    min-width: 1200px;
    padding: 5% 0;
    margin: 0 auto;
  }

  .title {
    margin: 80px 0 40px;
    text-align: center;
    font-size: 36px;
    color: #fff;
    &:first-child {
      margin: 0 0 40px;
    }
  }

  .process {
    ul {
      display: flex;
      justify-content: space-around;
      list-style-type: none;
      li {
        display: flex;
        position: relative;
        flex-direction: column;
        align-items: center;
        flex: 0 0 20%;
        p {
          font-size: 16px;
          line-height: 30px;
          color: #fff;
        }
        &:after {
          position: absolute;
          left: calc(50% + 22px);
          top: 22px;
          content: '';
          width: calc(100% - 44px);
          border-top: 2px solid #23c132;
        }
        &:nth-last-child(1):after {
          border: none;
        }
        .step {
          margin-bottom: 14px;
          border: 2px solid #23c132;
          width: 44px;
          height: 44px;
          font-size: 20px;
          color: #23c132;
          text-align: center;
          line-height: 44px;
          border-radius: 50%;
        }
      }
    }
  }

  .request ul {
    display: flex;
    justify-content: space-around;
    list-style-type: none;
    li {
      position: relative;
      flex: 0 0 30%;
      padding-bottom: 15%;
      width: 30%;
      background-color: #123354;
      img {
        position: absolute;
        width: 100%;
        height: 100%;
      }
    }
  }

  .standard {
    text-align: center;
    font-size: 16px;
    color: rgba(255, 255, 255, .6);
    line-height: 36px;
    .standard__title {
      margin: 50px 0 40px 0;
      font-size: 24px;
      color: #fff;
      line-height: 33px;
    }
    ul {
      display: flex;
      justify-content: space-around;
      list-style-type: none;
      li {
        position: relative;
        padding-bottom: 15%;
        width: 30%;
        flex: 0 0 30%;
        background-color: #123354;
        img {
          position: absolute;
          left: 0;
          top: 0;
          width: 100%;
          height: 100%;
        }
      }
    }
  }

  .title-standard {
    margin-bottom: 15px;
  }

  .process__data {
    margin-top: -10px;
    font-size: 16px;
    line-height: 36px;
    color: rgba(255, 255, 255, .6);
    a {
      font-size: 16px;
      text-decoration: underline;
      cursor: pointer;
      color: rgba(255, 255, 255, .6);
    }
  }

  .award {
    padding-top: 80px;
    margin: 0 auto;
    width: 62.5%;
    min-width: 1200px;
    ul {
      width: 100%;
      margin-bottom: 50px;
      list-style-type: none;
      display: flex;
      justify-content: flex-start;
      li {
        flex: 0 0 380px;
        display: inline-block;
        position: relative;
        width: 380px;
        margin-right: 50px;
        padding-bottom: 240px;
        img {
          position: absolute;
          width: 100%;
          height: 100%;
        }
      }
    }
  }

  .judge {
    width: 62.5%;
    min-width: 1200px;
    margin: 0 auto 70px;
    .judge__man {
      list-style-type: none;
      margin: 0 -30px;
      li {
        box-sizing: border-box;
        width: 50%;
        margin-bottom: 40px;
        padding: 0 30px;
        float: left;
        div {
          position: relative;
          width: 100%;
          padding-bottom: 45.6%;
          img {
            position: absolute;
            width: 100%;
            height: 100%;
          }
        }
      }
    }
  }

  .judge__rule {
    margin-top: 50px;
    p {
      text-align: center;
      margin-bottom: 24px;
      font-size: 24px;
      color: #fff;
    }
    ul {
      width: 680px;
      margin: 0 auto;
      list-style-type: none;
      li {
        margin: 0 auto;
        position: relative;
        color: rgba(255, 255, 255, .6);
        line-height: 36px;
        font-size: 16px;
        &:before {
          position: absolute;
          top: 15px;
          left: -18px;
          content: " ";
          width: 6px;
          height: 6px;
          background: #23c132;
          border-radius: 50%;
        }
      }
    }
  }

  .organizer_wrapper {
    width: 100%;
    background: #0E2C49;
  }

  .organizer {
    width: 62.5%;
    min-width: 1200px;
    margin: 0 auto;
    padding: 90px 0;
    .organize {
      position: relative;
      width: 224px;
      height: 90px;
      background: #fff;
      border: 1px solid #d8d8d8;
      border-radius: 4px;
      margin: 0 auto;
      img {
        position: absolute;
        width: 100%;
        height: 100%;
      }
    }
    ul {
      display: flex;
      justify-content: center;
      list-style-type: none;
      li {
        box-sizing: border-box;
        width: 19.5%;
        margin: 0 10px;
        display: inline-block;
      }
    }
  }

</style>
